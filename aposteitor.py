...
class Round():
	...
	def exist_competitors(self, competitors):
		...
		for competitor in competitors:
			if competitor in self.competitors or [x for x in self.competitors if x.name == competitor.name]:
				return True
		return False
	...
	def add_competitor(self, competitor):
		"""
		Adds a competitor to list of the round.
		...
		if self.exist_competitors([competitor]):
			raise NameError("El competidor ya está incluido en la ronda o ya existe un competidor con el mismo nombre")
	...
	def generate_padding_bets(self, amount):
		...
		for x in range(1,amount):
			...
			self.register_bet(pnj, pnj.generate_amount(), random.choice(self.competitors))